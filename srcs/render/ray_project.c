...
/*   Updated: 2026/04/17 15:25:58 by loena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_wall_height(t_game *game, t_ray *ray)
{
	if (ray->side == 0)
	{
		...
		ray->perp_dist += (1 - ray->step_x) / 2.0;
		ray->perp_dist /= ray->ray_dir_x;
	}
	else
	{
		ray->perp_dist = ray->map_y - game->player.pos_y;
		ray->perp_dist += (1 - ray->step_y) / 2.0;
		ray->perp_dist /= ray->ray_dir_y;
	}
